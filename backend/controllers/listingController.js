...
const listingSchema = Joi.object({
  ...
  pet_policy:Joi.string()
    .valid('no_pets', 'cats_only', 'dogs_only', 'pets_allowed')
    .required(),
...
const createListing = async (req, res) => {
    ...
    /*const result = await pool.query(
      `INSERT INTO properties (
        property_id, landlord_id, address_line, city, parish, rental_type,
         monthly_rent, is_available, description, verification_status, created_at
  ...
  const result = await pool.query(
    `INSERT INTO properties (
      landlord_id, address_line, city, parish, rental_type, bathrooms, bedrooms, 
     monthly_rent, availability_date, description,amenities, square_footage, pet_policy,  created_at
...
const partialUpdateSchema = Joi.object({
  ...
  availability_date: Joi.date().optional(),
...
const getListingById = async (req, res) => {
  try {
   ...
   //const listing = listingResult.rows[0];
  //listing.images = imagesResult.rows.map(row => row.file_path); // for image paths only
...
const getListingsByLandlord = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      const listingsWithImages = await Promise.all(result.rows.map(async (listing) => {
      const images = await pool.query(
        `SELECT file_path FROM gallery WHERE property_id = $1 LIMIT 1`,
        [listing.property_id]
      );
      return {
        ...listing,
...
const searchListings = async (req, res) => {
  try {
   ...
   // }
    ...
    if (pet_policy) {
      values.push(pet_policy);
      conditions.push(`pet_policy = $${values.length}`);
    }


    if (square_footage) {
    values.push(parseInt(square_footage));
    conditions.push(`square_footage >= $${values.length}`);
  }
    ...
    if (parish) {
      values.push(parish);
      conditions.push(`parish = $${values.length}`);
    }

    if (rental_type) {
      values.push(rental_type);
      conditions.push(`rental_type = $${values.length}`);
    }

    // Optional: include this only if furnished filtering will be added
    // if (furnished !== undefined) {
    //   const furnishedBool = furnished === 'true';
    //   values.push(furnishedBool);
    //   conditions.push(`furnished = $${values.length}`);
    // }

    const parsedBedrooms = parseInt(bedrooms);
    if (!isNaN(parsedBedrooms)) {
      values.push(parsedBedrooms);
      conditions.push(`bedrooms >= $${values.length}`);
    }

    const parsedBathrooms = parseInt(bathrooms);
    if (!isNaN(parsedBathrooms)) {
      values.push(parsedBathrooms);
      conditions.push(`bathrooms >= $${values.length}`);
    }
    ...
    const parsedSquareFootage = parseInt(square_footage);
    if (!isNaN(parsedSquareFootage)) {
      values.push(parsedSquareFootage);
      conditions.push(`square_footage >= $${values.length}`);
    }

    const parsedMinRent = Number(minRent);
    if (!isNaN(parsedMinRent)) {
      values.push(parsedMinRent);
      conditions.push(`monthly_rent >= $${values.length}`);
    }

    const parsedMaxRent = Number(maxRent);
    if (!isNaN(parsedMaxRent)) {
      values.push(parsedMaxRent);
      conditions.push(`monthly_rent <= $${values.length}`);
    }
    ...
    // 🖼 Attach 1 image to each listing
    const listingsWithImages = await Promise.all(
      result.rows.map(async (listing) => {
        ...
        return {
          ...listing,
          images: imageRes.rows // image URL will be used on frontend
        };
      })
    );
